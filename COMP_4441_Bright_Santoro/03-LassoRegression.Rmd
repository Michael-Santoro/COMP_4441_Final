...
RMSE.1 <- RMSE(predicted,test$quality)
...
cv_fit <- cv.glmnet(train.mat, qual, alpha = 0, lambda = lambdas)
plot(cv_fit)
```
...
```{r}
rr <- predict(fit, s = opt_lambda, newx = test.mat)

RMSE.4 <- RMSE(rr,test$quality)
RMSE.4
```
